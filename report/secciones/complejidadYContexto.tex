...
El problema en cuestión esta contenido en la clase de problemas NP-completo. Se argumenta su pertenencia a la clase NP y su NP completitud.

\subsection{Pertenencia a \textit{NP}}

Un problema pertenece a la clase \textit{NP} si, dadas las soluciones, es posible verificar si estas soluciones son correctas en tiempo polinomial. El problema se puede ejemplificar suponiendo que se tiene como solución un conjunto de \textit{K} subconjuntos, que son los que particionan el conjunto de los vértices \textit{V}. Para verificar que una solución dada cumple con la propiedad de solución del problema, la solución debe cumplir las siguientes condiciones:
...
Dado que el problema \textit{Graph Coloring} es \textit{NP-completo} y existe una transformación polinomial entre este y el problema de \textit{Clique Cover}, se concluye que \textit{Clique Cover} es \textit{NP-duro}.

Como además se demostró anteriormente que dicho problema pertenece a \textit{NP} es posible llegar a la conclusión de que \textit{Clique Cover} es \textit{NP-completo}.
...
\\
\vspace{0.15cm}
\small \textit{Fuente: Elaboración propia basada en Computers and Intractability: A Guide to the Theory of NP-Completeness \cite{garey1979}.}
\end{table}
